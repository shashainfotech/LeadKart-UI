...
import CheckBox from './common-components/CardComponent.vue';
import { ref } from 'vue';
...
<template>
    <PageHeading title="Checkbox & Radio" sub-title="Form" />
    <h6 class="mb-2 font-semibold text-15">Checkbox</h6>
    <div class="grid grid-cols-12 gap-x-space">
        <CheckBox title="Basic">
            <div v-for="(color, index) in colors" :key="index" class="input-check-group">
                <input :id="'checkboxBasic' + (index + 1)" class="input-check" :class="color" type="checkbox" />
                <label :for="'checkboxBasic' + (index + 1)" class="input-check-label">
                    {{ capitalize(color) }}
                </label>
            </div>
        </CheckBox>
        <CheckBox title="Circle">
            <div v-for="(color, index) in circleColors" :key="index" class="input-check-group">
                <input :id="'checkboxOutline' + (index + 1)" class="rounded-full input-check" :class="color"
                    type="checkbox" />
                <label :for="'checkboxOutline' + (index + 1)" class="input-check-label">
                    {{ capitalize(color) }}
                </label>
            </div>
        </CheckBox>
        <CheckBox title="Soft Checkbox">
            <div v-for="(color, index) in softColors" :key="index" class="input-check-group">
                <input :id="'checkboxSoft' + (index + 1)" class="input-check" :class="color" type="checkbox" />
                <label :for="'checkboxSoft' + (index + 1)" class="input-check-label">
                    {{ getLabel(color) }}
                </label>
            </div>
        </CheckBox>
    </div>
    <h6 class="mb-2 font-semibold text-15">Radio Example</h6>
    <div class="grid grid-cols-12 gap-x-5">
        <CheckBox title="Basic" extra-css="space-y-3 ">
            <div v-for="(color, index) in radioColors" :key="index" class="input-radio-group">
                <input :id="'radioBasic' + (index + 1)" class="input-radio" :class="color.className" type="radio"
                    :name="color.name" />
                <label :for="'radioBasic' + (index + 1)" class="capitalize input-radio-label">
                    {{ color.name }}
                </label>
            </div>
        </CheckBox>
        <CheckBox title="Square" extra-css="space-y-3 ">
            <div v-for="(radio, index) in radioColors" :key="index" class="input-radio-group">
                <input :id="'radioBasic' + (index + 11)" class="rounded-none input-radio" :class="radio.className"
                    type="radio" :name="'square' + radio.name" />
                <label :for="'radioBasic' + (index + 11)" class="input-radio-label">
                    {{ radio.name }}
                </label>
            </div>
        </CheckBox>
        <CheckBox title="Soft Checkbox" extra-css="space-y-3 ">
            <div v-for="(radio, index) in softRadioColors" :key="index" class="input-radio-group">
                <input :id="'radioBasic' + (index + 21)" class="input-radio" :class="radio.className" type="radio" />
                <label :for="'radioBasic' + (index + 21)" class="input-radio-label">
                    {{ radio.name }}
                </label>
            </div>
        </CheckBox>
    ...
    <div class="grid grid-cols-12 gap-x-5">
        <CheckBox title="Card Selected" extra-css="space-y-2">
            <template v-for="(method, index) in paymentMethod" :key="index">
                <label :for="method.name"
                    class="has-[:checked]:ring-primary-500/20 cursor-pointer has-[:checked]:text-primary-500 has-[:checked]:bg-primary-500/10 grid grid-cols-[24px_1fr_auto] items-center gap-6 rounded-lg p-4 ring-1 ring-transparent hover:bg-gray-100 dark:hover:bg-dark-850">
                    <svg class="w-8" fill="none" viewBox="0 0 24 13">
                        <rect v-if="method.name.toLowerCase().includes('card')" stroke="currentColor" x="0.5" y="0.5"
                            width="23" height="11" rx="1.5"></rect>
                        <path fill="currentColor" :d="method.path">
                        </path>
                    </svg>
                    {{ method.name }}
                    <input name="payment_method" :id="method.name" value="google" type="radio"
                        class="input-radio input-radio-primary" :checked="method.checked">
                </label>
            </template>
        </CheckBox>
        <CheckBox title="Do To List" extra-css="space-y-3">
            <legend class="text-base font-semibold text-gray-900 dark:text-gray-200">Today</legend>
            <div v-for="(task, index) in tasks" :key="index" class="grid grid-cols-[1fr_24px] items-center gap-6">
                <label class="peer grid grid-cols-[auto_1fr] items-center gap-3 rounded-md px-2 cursor-pointer">
                    <input type="checkbox" class="input-check peer input-check-red" v-model="task.checked">
                    <span
                        class="select-none peer-checked:text-gray-500 dark:peer-checked:text-dark-500 peer-checked:line-through">
                        {{ task.name }}
                    </span>
                </label>
                <a href="#!"
                    class="size-5 rounded-md flex items-center justify-center text-gray-500 dark:text-dark-500 hover:bg-red-500/10 hover:text-red-500 dark:hover:text-red-500 peer-has-[:checked]:hidden"
